...
class ResNet18_fire_model(LightningModule):
    """
    ResNet18-based model for fire prediction with spatial data.
    """
    ...
    def __init__(
        ...
        # Match other models (NLLLoss expects log-probs)
        class_weights = torch.tensor([1. - positive_weight, positive_weight])
        self.register_buffer("class_weights", class_weights)
        self.criterion = torch.nn.NLLLoss(weight=self.class_weights)
...
class ViT_fire_model(LightningModule):
    def __init__(
            self,
            dynamic_features=None,
            static_features=None,
